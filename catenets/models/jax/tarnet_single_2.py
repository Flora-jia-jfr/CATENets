...
class TARNet_single_2(BaseCATENet):
    """
    Class implements modified version_2 of TARNet, changing TARNet from 2 branches to 1 branch
    used to verify the importance of causal componenet
    concat treatment to representation
    ...
    def _get_train_function(self) -> Callable:
        return train_tarnet_single_2

    def _get_predict_function(self) -> Callable:
        return predict_tarnet_single_2
...
def predict_tarnet_single_2(
    ...
    # get representations
    reps = predict_fun_repr(param_repr, X)

    reps_concat_w_0 = jnp.concatenate((reps, w_0), axis=1)
    reps_concat_w_1 = jnp.concatenate((reps, w_1), axis=1)

    # get potential outcomes
    mu_0 = predict_fun_head(param_single, reps_concat_w_0)
    mu_1 = predict_fun_head(param_single, reps_concat_w_1)
...
def train_tarnet_single_2(
    ...
    def init_fun_snet1(rng: float, input_shape: Tuple) -> Tuple[Tuple, List]:
        ...
        batch = input_shape_repr[0]
        dim  = input_shape_repr[1] + 1
        input_shape, param_single = init_fun_head(layer_rng, (batch, dim))
